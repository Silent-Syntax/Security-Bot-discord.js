...
module.exports = {
	...
	execute(message, client) {
	 ...
	 if (!db1.get({key: `${message.guild.id}_CONTROLUSERS`}).includes(message.author.id) && message.author.id !== message.guild.ownerId) return interaction.reply({content: '**❌ Only Ownership And Control Users Can Use This Command**'})
...
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
      ...
      .setTimestamp()
			...
			message.reply({content: `**✅ Antilinks is enabled**`})
...
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
      ...
      .setTimestamp()
			...
			message.reply({content: `**✅ Antilinks is disabled**`}) 
