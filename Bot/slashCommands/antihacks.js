...
module.exports = {
	...
	options: [{
		name: "value",
		description: "type status of antihacks protection",
		type: "STRING",
		required: true,
		choices: [{
			name: "on",
...
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
      ...
      .setTimestamp()
			...
			db.set(`ANTIHACKS_${interaction.guild.id}`, "true")

			interaction.reply({content: `**✅ Antihacks is enabled**`})
...
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
      ...
      .setTimestamp()
			...
			db.set(`ANTIHACKS_${interaction.guild.id}`, "false")

			interaction.reply({content: `**✅ Antihacks is disabled**`}) 
